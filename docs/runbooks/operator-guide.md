...
This runbook is for the Linux terminal/paper deployment described by `PLAN.md`.
...
1. Install the pinned Rust toolchain declared in `rust-toolchain.toml` and Python
   runtime declared in `.python-version`. On Debian/Ubuntu reference hosts:

   ```bash
   sudo apt-get install --yes protobuf-compiler libssl-dev
   ```

   Record the package-manager output in release evidence.
2. Build from the Rust lockfile, then run the repository gate:
...
Edit only deployment-approved values or use `CONFIG LOAD <path>` in the terminal to
atomically install validated values. Keep `broker.mode = "paper"`; API keys belong in the secret
manager/environment. Validate the file through the runtime before starting
...
In another terminal, run `insider-terminal --socket data/runtime.sock`. Confirm in
`HEALTH` that the journal, market feed, broker session, risk
...
`CONFIRM`, and fill reconciliation are visible. A terminal crash must not be treated as
...
1. Stop the terminal independently; verify the execution service remains in its configured
   deployment mode.
...
Use the authenticated backup command to create a
...
2. Create a new credential in the deployment secret manager. Do not write the
   secret, token, or key into `.cfg`, terminal history, journals, traces, or shell history.
